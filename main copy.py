...
class VideoCamera:
    ...
    def start(self):
        if not self.is_running:
            # Try multiple indices. CAP_DSHOW is highly recommended on Windows for USB cams.
            # We prioritize 1 and 2 for external, then 0 for internal, then fallbacks.
            for idx in [1, 2, 0, 3, 4]:
                print(f"DEBUG: Attempting to open camera index {idx} using DirectShow...")
                # Using CAP_DSHOW to prevent hangs on Windows
                self.video = cv2.VideoCapture(idx, cv2.CAP_DSHOW)
                
                if self.video.isOpened():
                    # Double check if the camera actually produces frames
                    ret, _ = self.video.read()
                    if ret:
                        print(f"DEBUG: Successfully verified camera index {idx}")
                        self.video.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
                        self.video.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
                        self.is_running = True
                        stabilizer.reset()
                        ...
                        print(f"DEBUG: Camera index {idx} opened but failed to read frame.")
                    
                if self.video:
                    self.video.release()
                print(f"DEBUG: Index {idx} not available.")
            
            print("ERROR: Could not find a working camera device.")
            return False
...
def start_camera():
    ...
    if success:
        ...
        return jsonify({'status': 'error', 'message': 'Could not open camera device'})
